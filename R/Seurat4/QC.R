...
QC_list <- lapply(df_samples$sample.id, function(x){
        tmp = read.csv(file = paste0("data/",x,
...
openxlsx::write.xlsx(df_samples, file =  paste0(path,"20210525_scRNAseq_info~.xlsx"),
                     colNames = TRUE,row.names = T,borders = "surrounding",colWidths = c(NA, "auto", "auto"))

## Load the GEX dataset
message("Loading the datasets")
df_samples %<>% filter(sequencing == "GEX")
Seurat_list <- pblapply(df_samples$sample.id, function(s){
        tmp <- Read10X(data.dir = paste0("data/",as.character(s),"/outs/filtered_feature_bc_matrix"))
...
object$orig.ident = gsub("-GEX","",colnames(object)) %>% 
        gsub(".*-","",.) %>% 
        gsub(".*_","",.)
